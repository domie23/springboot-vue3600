		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="导师工号" prop="tutor_id">
												<el-input id="tutor_id" v-model="form_sub['tutor_id']" placeholder="请输入导师工号"
							  v-if="user_group === '管理员' || (form_sub['tutor_user_id'] && $check_field('set','tutor_id')) || (!form_sub['tutor_user_id'] && $check_field('add','tutor_id'))"></el-input>
					<div v-else-if="$check_field('get','tutor_id')">{{form_sub['tutor_id']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="导师姓名" prop="tutor_name">
												<el-input id="tutor_name" v-model="form_sub['tutor_name']" placeholder="请输入导师姓名"
							  v-if="user_group === '管理员' || (form_sub['tutor_user_id'] && $check_field('set','tutor_name')) || (!form_sub['tutor_user_id'] && $check_field('add','tutor_name'))"></el-input>
					<div v-else-if="$check_field('get','tutor_name')">{{form_sub['tutor_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','mentor_gender') || $check_field('add','mentor_gender') || $check_field('set','mentor_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="导师性别" prop="mentor_gender">
								<el-select id="mentor_gender" v-model="form_sub['mentor_gender']"
						v-if="user_group === '管理员' || (form_sub['tutor_user_id'] && $check_field('set','mentor_gender')) || (!form_sub['tutor_user_id'] && $check_field('add','mentor_gender'))">
						<el-option v-for="o in list_mentor_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','mentor_gender')">{{form_sub['mentor_gender']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','tutor_age') || $check_field('add','tutor_age') || $check_field('set','tutor_age')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="导师年龄" prop="tutor_age">
												<el-input id="tutor_age" v-model="form_sub['tutor_age']" placeholder="请输入导师年龄"
							  v-if="user_group === '管理员' || (form_sub['tutor_user_id'] && $check_field('set','tutor_age')) || (!form_sub['tutor_user_id'] && $check_field('add','tutor_age'))"></el-input>
					<div v-else-if="$check_field('get','tutor_age')">{{form_sub['tutor_age']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
			...
			return {
				...
				disabledObj:{
								"tutor_id_isDisabled": false,
										...
										"mentor_gender_isDisabled": false,
										"tutor_age_isDisabled": false,
							},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["tutor_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?tutor_user_id=" + form_sub["tutor_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/tutor_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_user/view','get');
