	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="学生信息" prop="student_information">
							...
							<!--v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','student_information')) || (!form['tutor_answers_questions_id'] && $check_field('add','student_information'))" :disabled="disabledObj['student_information_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_information')">{{form['student_information']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','student_information')) || (!form['tutor_answers_questions_id'] && $check_field('add','student_information'))" id="student_information" v-model="form['student_information']" :disabled="disabledObj['student_information_isDisabled']">
				...
				<el-form-item label="学生学号" prop="student_id">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','student_id')) || (!form['tutor_answers_questions_id'] && $check_field('add','student_id'))" :disabled="disabledObj['student_id_isDisabled']"></el-input>
				...
				<el-form-item label="学生姓名" prop="student_name">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','student_name')) || (!form['tutor_answers_questions_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
				...
				<el-form-item label="班级名称" prop="class_name">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','class_name')) || (!form['tutor_answers_questions_id'] && $check_field('add','class_name'))" :disabled="disabledObj['class_name_isDisabled']"></el-input>
				...
				<el-form-item label="导师信息" prop="tutor_information">
													<el-select v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','tutor_information')) || (!form['tutor_answers_questions_id'] && $check_field('add','tutor_information'))" id="tutor_information" v-model="form['tutor_information']" :disabled="disabledObj['tutor_information_isDisabled']">
				...
				<el-form-item label="导师工号" prop="tutor_id">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','tutor_id')) || (!form['tutor_answers_questions_id'] && $check_field('add','tutor_id'))" :disabled="disabledObj['tutor_id_isDisabled']"></el-input>
				...
				<el-form-item label="导师姓名" prop="tutor_name">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','tutor_name')) || (!form['tutor_answers_questions_id'] && $check_field('add','tutor_name'))" :disabled="disabledObj['tutor_name_isDisabled']"></el-input>
				...
				<el-form-item label="论文题目" prop="thesis_title">
							  ...
							  v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','thesis_title')) || (!form['tutor_answers_questions_id'] && $check_field('add','thesis_title'))" :disabled="disabledObj['thesis_title_isDisabled']"></el-input>
				...
				<el-form-item label="留言内容" prop="message_content">
						...
						v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','message_content')) || (!form['tutor_answers_questions_id'] && $check_field('add','message_content'))" :disabled="disabledObj['message_content_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','message_content')">{{form['message_content']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','content_reply') || $check_field('add','content_reply') || $check_field('set','content_reply')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="内容回复" prop="content_reply">
								<el-input type="textarea" id="content_reply" v-model="form['content_reply']" placeholder="请输入内容回复"
						v-if="user_group === '管理员' || (form['tutor_answers_questions_id'] && $check_field('set','content_reply')) || (!form['tutor_answers_questions_id'] && $check_field('add','content_reply'))" :disabled="disabledObj['content_reply_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','content_reply')">{{form['content_reply']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "tutor_answers_questions_id",
				url_add: "~/api/tutor_answers_questions/add?",
				url_set: "~/api/tutor_answers_questions/set?",
				url_get_obj: "~/api/tutor_answers_questions/get_obj?",
				url_upload: "~/api/tutor_answers_questions/upload?",

				query: {
					"tutor_answers_questions_id": 0,
				},
				...
				form: {
								"student_information": 0, // 学生信息
										...
										"content_reply":  '', // 内容回复
											"tutor_answers_questions_id": 0, // ID
				...
				disabledObj:{
								"student_information_isDisabled": false,
										...
										"content_reply_isDisabled": false,
										},
		...
		methods: {
					...
					if (res.result && res.result.obj) {
						...
						var arrForm = []
							...
							arrForm.push(key)
												...
												_this.form["student_information"] = id
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/tutor_answers_questions/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_answers_questions/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_answers_questions/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_answers_questions/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/tutor_answers_questions/view','get');
