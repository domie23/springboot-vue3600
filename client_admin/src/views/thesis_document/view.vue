	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="学生信息" prop="student_information">
							...
							<!--v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','student_information')) || (!form['thesis_document_id'] && $check_field('add','student_information'))" :disabled="disabledObj['student_information_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_information')">{{form['student_information']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','student_information')) || (!form['thesis_document_id'] && $check_field('add','student_information'))" id="student_information" v-model="form['student_information']" :disabled="disabledObj['student_information_isDisabled']">
				...
				<el-form-item label="学生学号" prop="student_id">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','student_id')) || (!form['thesis_document_id'] && $check_field('add','student_id'))" :disabled="disabledObj['student_id_isDisabled']"></el-input>
				...
				<el-form-item label="学生姓名" prop="student_name">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','student_name')) || (!form['thesis_document_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
				...
				<el-form-item label="班级名称" prop="class_name">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','class_name')) || (!form['thesis_document_id'] && $check_field('add','class_name'))" :disabled="disabledObj['class_name_isDisabled']"></el-input>
				...
				<el-form-item label="导师信息" prop="tutor_information">
													<el-select v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','tutor_information')) || (!form['thesis_document_id'] && $check_field('add','tutor_information'))" id="tutor_information" v-model="form['tutor_information']" :disabled="disabledObj['tutor_information_isDisabled']">
				...
				<el-form-item label="导师工号" prop="tutor_id">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','tutor_id')) || (!form['thesis_document_id'] && $check_field('add','tutor_id'))" :disabled="disabledObj['tutor_id_isDisabled']"></el-input>
				...
				<el-form-item label="导师姓名" prop="tutor_name">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','tutor_name')) || (!form['thesis_document_id'] && $check_field('add','tutor_name'))" :disabled="disabledObj['tutor_name_isDisabled']"></el-input>
				...
				<el-form-item label="论文题目" prop="thesis_title">
							  ...
							  v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','thesis_title')) || (!form['thesis_document_id'] && $check_field('add','thesis_title'))" :disabled="disabledObj['thesis_title_isDisabled']"></el-input>
				...
				<el-form-item label="文档类型" prop="document_type">
						...
						v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','document_type')) || (!form['thesis_document_id'] && $check_field('add','document_type'))">
					...
					<div v-else-if="$check_field('get','document_type')">{{form['document_type']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','upload_document') || $check_field('add','upload_document') || $check_field('set','upload_document')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="上传文档" prop="upload_document">
												<div v-if="disabledObj['upload_document_isDisabled']">
						<div v-if="$check_field('get','upload_document')">
							<el-button type="primary" @click="$download($fullUrl(form['upload_document']),form['upload_document'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['thesis_document_id'] && $check_field('set','upload_document')) || (!form['thesis_document_id'] && $check_field('add','upload_document'))" class="upload-demo" drag
								   action="" style="max-width: 300px;width: 100%;" :http-request="upload_upload_document" :limit="1" accept="">
							<i class="el-icon-upload"></i>
							<div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
						</el-upload>
						<div v-else-if="$check_field('get','upload_document')">
							<el-button type="primary" @click="$download($fullUrl(form['upload_document']),form['upload_document'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/thesis_document/add?",
				url_set: "~/api/thesis_document/set?",
				url_get_obj: "~/api/thesis_document/get_obj?",
				url_upload: "~/api/thesis_document/upload?",

				query: {
					"thesis_document_id": 0,
				},
				...
				disabledObj:{
								"student_information_isDisabled": false,
										...
										"upload_document_isDisabled": false,
										},
		...
		methods: {
			 ...
			 */
			upload_upload_document(param){
						this.uploadFile(param.file, "upload_document");
					},
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/thesis_document/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/thesis_document/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/thesis_document/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/thesis_document/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/thesis_document/view','get');
